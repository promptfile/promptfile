...
Use the `<Functions>{:prompt}` block to define a tool that the LLM is able to use. The LLM will return a response with the name of the tool it decides to use and the arguments for the schema you have defined.

A `<Function>{:prompt}` block has the same JSON structure as defined by OpenAI. Read more about it [here](https://platform.openai.com/docs/guides/gpt/function-calling). 

Note that this block is only valid in specific OpenAI models. Currently, only `gpt-4-0613` and `gpt-3.5-turbo-0613` support tool usage.

In the example below, we define a function called `calculator`. We also input a `testValue` that our function (which would be implemented elsewhere in our project) would return.
...
The `<Function>{:prompt}` block is automatically generated and used to track the results of a function evaluation.
