...
To enable dynamic conversations with your prompt, Promptfile automatically adds a `<User>{:glass}` block containing an uninitialized input variable, along with a `<Request>{:glass}` block with the same arguments found in the most recent request.
For example:
