...
Here you can declare variables that you can interpolate into your prompt. You can declare variables in the `<Init>{:glass}` block and then use them in your prompt with the `@{variableNameHere}` syntax. All variables must be inside a block. If you want the user to provide the variable, simply don't initialize the variable in an `<Init>{:glass}`.
...
Using an `<Init>{:glass}` block, you can execute code to do things like access a user's profile. In this example, you can see how you could use a user profile in our prompt to make the conversation more personal.
