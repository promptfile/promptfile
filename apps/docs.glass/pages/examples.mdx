...
The Glass Cookbook provides a collection of practical examples that demonstrate Glass' capabilities.
...
To declare variables for interpolation within your prompt, utilize the `<Code>{:glass}` block. Use the `@{variableName}` syntax to use those variables in your prompt. Ensure that all variables are placed within a block. If you want the user to define the variable when interacting, do not declare it. The Playground will automatically as the user to define those undeclared variables.
...
You may also retrieve information from external sources, such as a database or website. In this example, we demonstrate fetching HTML content from a website and incorporating it into our prompt using an `<Code>{:glass}` block. To integrate more dynamic information into the conversation, consider making API requests or loading external data.
...
Employ the `<Tool>{:glass}` block to create a tool (or function) that enhances the model's ability to perform tasks. In this case, a calculator is provided as a resource for the LLM to use in accomplishing its task.
...
LLMs excel in comprehending semantic meaning and text classification. This example highlights the `<For>{:glass}` block, which enables iteration through an array of examples. Here, we iterate over an array of comments and their corresponding classifications.
