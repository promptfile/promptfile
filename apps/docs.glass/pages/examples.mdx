...
The Glass Cookbook provides a collection of practical examples to assist you in getting started with Glass.

<Steps>

### Declaring Variables

To declare variables for interpolation within your prompt, utilize the `<Init>{:glass}` block. Follow the syntax `@{variableName}` to use them in your prompt. Ensure that all variables are placed within a block. If you require user input for the variable, simply do not declare it. The Playground will automatically request user input for those undeclared variables.
...
You may also retrieve information from external sources, such as a database or website. In this example, we demonstrate fetching HTML content from a website and incorporating it into our prompt using an `<Init>{:glass}` block. To seamlessly integrate dynamic information into the conversation, consider making API requests or loading external data.
...
Employ the `<Tool>{:glass}` block to create a tool (or function) that enhances the model's ability to perform tasks. In this case, a calculator is provided as a resource for the LLM to utilize.

Define the tool by specifying its parameters, which include the tool itself, the schema the LLM needs to return to execute the function, and the intended function to run. In this example, we use [Zod](https://zod.dev/) to parse and validate the data.
...
For complex prompts, achieving the desired outcome often requires executing multiple requests. In this example, we will link two prompts to generate both a synopsis of a fictitious play and a review of that fictitious play.

<Callout type="info"> Observe the utilization of the `<Test>{:glass}` block, replacing the `<Init>{:glass}` block. This creates a test block that passes the test case as arguments when running the file. However, when implementing the Glass file within the actual project, the arguments can be supplied as parameters during invocation. </Callout>

```glass copy filename="promptChaining.glass"
<Test>
const title = 'The Engineer Who Built Glass'
</Test>
...
Leverage the useState hook to introduce state management in your chat. This enables tracking features such as the total number of messages and the most recent message sent by the user.
...
The `<State>{:glass}` is generated automatically to a file to keep track of the state of a chat. Additionally, observe the onResponse attribute, which permits code execution based on the LLM request's response. In this situation, we use the LLM request's response to update the last message sent by the user.
...
LLMs excel in comprehending semantic meaning and text classification. This example highlights the `<For>{:glass}` block, which enables iteration through an array of examples. Here, we iterate over an array of comments and their corresponding classifications. By incorporating the examples variable in our prompt, we efficiently classify the user's comment.
