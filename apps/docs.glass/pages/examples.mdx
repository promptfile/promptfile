...
The Glass Cookbook includes examples to help you get started with Glass. You can also find examples in the demo [folder](https://github.com/glass-lang/glass/tree/main/apps/demo/examples) of the project.

<Steps>

### Add Variables

Here you can declare variables that you can interpolate into your prompt. You can declare variables in the `<Code>{:glass}` block and then use them in your prompt with the `@{variableNameHere}` syntax. All variables must be inside a block. If you want the user to provide the variable, simply don't initialize the variable in an `<Code>{:glass}`.
...
Using an `<Code>{:glass}` block, you can execute code to do things like access a user's profile. In this example, you can see how you could use a user profile in our prompt to make the conversation more personal.
...
You can define a tool (or function) that the model can now use to accomplish its task. In this case, we give it the ability to use a Calculator. We define what the tool is called via `name`, what it does via `description`, and then we provide a schema (in this case, we are using [Zod](https://zod.dev/) to validate the output) and then evaluating it directly.

Keep in mind, currently only `gpt-3.5-turbo-0613` and `gpt-4-0613` have been fine-tuned on calling functions.
...
<Callout type="info"> Notice the use of `<Test>{:glass}` block instead of `<Code>{:glass}`. Here we create a test block, which means that when we run the file, it will pass in our test case as arguments, but when we actually use it in our project, we can pass our arguments in as parameters to invoking the Glass file.</Callout>
