...
If it's your first time executing a Glass file, you'll be asked to input the key for the specific model you are using in [VSCode settings](/vscode). All of Glass runs locally on your machine — your secret keys are never sent to any server. In this case, we're using the `gpt-3.5-turbo` model, so we'll need to input the key for OpenAI `glass.openaiKey`.
