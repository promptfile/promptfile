...
export const SessionView = (props: SessionViewProps) => {
  const { blocks, streaming, runChat, stop, theme, session, inputs, setValue } = props
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      {Object.keys(inputs).length > 0 ? (
        ...
        >
          {Object.keys(inputs).map(key => (
            <div key={key} style={{ display: 'flex', flexDirection: 'column', paddingTop: '48px' }}>
      ...
      <ComposerView theme={theme} runChat={runChat} stop={stop} streaming={streaming} />
    </div>
  )
}
